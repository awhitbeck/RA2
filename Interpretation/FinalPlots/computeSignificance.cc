...
int main(int argc, char *argv[]) {
  if( argc != 2 ) {
    std::cerr << "Usage: ./computeSignificance <n tries>" << std::endl;
    return 1;
  }
  ...
  // Takes care of search-bin reading
  const SearchBinManager binManager("data",bkgs);

  for(std::vector<std::string>::const_iterator bkgIt = bkgs.begin();
      bkgIt != bkgs.end(); ++bkgIt) {
    // Read predictions
    std::vector<SearchBin*> bins = binManager.readSearchBins( *bkgIt );

    // add to ToyExperiment
