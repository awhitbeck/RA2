...
// $Id: ISRHistoWeightProducer.cc,v 1.1 2012/12/29 20:04:20 seema Exp $
//
//
  ...
  fReweight_(iConfig.getUntrackedParameter< std::string > ("reweightVariable"))
{
  ...
  produces<double>("ISRWeight");
  produces<double>("SusyMoSumPt");
  produces<double>("SusyMoSumEta");
  produces<double>("SusyMoSumPhi");
  produces<double>("SusyMoSumE");
}
...
void ISRHistoWeightProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  iEvent.put(outWeight, "ISRWeight");
 
  std::auto_ptr<double> moPt (new double((*pit).pt()));
  std::auto_ptr<double> moEta(new double((*pit).eta()));
  std::auto_ptr<double> moPhi(new double((*pit).phi()));
  std::auto_ptr<double> moE  (new double((*pit).energy()));
  iEvent.put(moPt, "SusyMoSumPt");
  iEvent.put(moEta,"SusyMoSumEta");
  iEvent.put(moPhi,"SusyMoSumPhi");
  iEvent.put(moE,  "SusyMoSumE");
}
